...
export async function GET(request: NextRequest) {
  ...
  const allRoles = await db.select({ userId: userRole.userId, role: userRole.role }).from(userRole)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const created = await db.select({ id: userTable.id, name: userTable.name, email: userTable.email, createdAt: userTable.createdAt })
      .from(userTable)
    ...
    if (body.phone && created[0]) {
      await db.update(userTable).set({ phone: body.phone }).where(eq(userTable.id, created[0].id))
    }
