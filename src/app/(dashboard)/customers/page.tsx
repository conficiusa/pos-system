...
export default function CustomersPage() {
  ...
  const [settlementSuccess, setSettlementSuccess] = useState<string | null>(
    null,
  );
  ...
  const handleSettle = async (e: React.FormEvent, balance: number) => {
    ...
    if (amount > Math.abs(balance)) {
      setSettlementError(
        `Cannot exceed outstanding balance of ${fmtGHS(Math.abs(balance))}`,
      );
      return;
    }
    setIsSettling(true);
    ...
    <DashboardShell activeItem="customers" user={sidebarUser}>
                  ...
                  <div className="p-5 space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {Array.from({ length: 3 }).map((_, i) => (
                        <span
                          key={i}
                          className="block h-12 animate-pulse rounded-md bg-pos-bg-secondary"
                        />
                      ))}
                    </div>
                  </div>
                      ...
                      {detail.recentOrders?.map((item) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            {item.ledgerAdjustment &&
                            item.ledgerAdjustment !== 0 ? (
                              <p
                                className={
                                  item.ledgerAdjustment > 0
                                    ? "text-pos-success"
                              ...
                              >
                                {item.ledgerAdjustment > 0 ? "+" : "-"}
                                {fmtGHS(Math.abs(item.ledgerAdjustment))}
                              </p>
                            ) : null}
                          </div>
                        </div>
                    ...
                    {(detail.customer.ledgerBalance ?? 0) !== 0 && (
                      <div className="border-t border-pos-border-tertiary px-5 py-4">
                        ...
                        >
                          ...
                          />
                          {settlementError && (
                            <p className="text-[12px] text-pos-danger">
                              {settlementError}
                            </p>
                          )}
                          {settlementSuccess && (
                            <p className="text-[12px] text-pos-success">
                              {settlementSuccess}
                            </p>
                          )}
                        </form>
      ...
      {showNewForm && (
        ...
        >
          ...
          >
            <h2 className="mb-4 text-[15px] font-medium text-pos-text-primary">
              New customer
            </h2>
