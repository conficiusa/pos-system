...
export default function ReportsPage() {
  ...
  const statsQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch("/api/reports/stats").then(
        (r) => r.json() as Promise<ReportsStats>,
      ),
  });
  ...
  const chartsQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch("/api/reports/charts").then(
        (r) => r.json() as Promise<ChartData>,
      ),
  });
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: stats
        ? String(
            stats.ledger.customersWithCredit + stats.ledger.customersWithDebit,
          )
        : "—",
        ...
        <div className="flex flex-1 flex-col gap-5 p-5">
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            {metrics.map((metric) => (
              ...
              >
                ...
                <p className="text-[11px] text-pos-text-tertiary">
                  {metric.sub}
                </p>
              </div>
            ))}
          ...
          <div className="overflow-hidden rounded-[var(--radius-lg)] border border-pos-border-tertiary bg-pos-bg-primary">
            <div className="flex items-center justify-between border-b border-pos-border-tertiary px-5 py-3">
              <p className="text-[13px] font-medium text-pos-text-primary">
                Payouts by week
              </p>
              ...
              {chartsQuery.isLoading ? (
                ...
                (() => {
                  const maxVal = Math.max(
                    ...chartData.weeklyPayouts.map((w) => w.totalPaid),
                  );
                  return (
                    <>
                      <div className="flex h-[120px] items-end gap-2">
                        {chartData.weeklyPayouts.map((item) => (
                          ...
                          >
                            ...
                            <div
                              ...
                              style={{
                                height: `${Math.round((item.totalPaid / maxVal) * 90)}px`,
                              }}
                            />
                          </div>
                        ))}
                      ...
                      <div className="mt-2 flex justify-between">
                        {chartData.weeklyPayouts.map((item) => (
                          <span
                            key={item.week}
                            className="text-[10px] text-pos-text-tertiary"
                          >
                            {item.week}
                          </span>
                        ))}
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <div className="overflow-hidden rounded-[var(--radius-lg)] border border-pos-border-tertiary bg-pos-bg-primary">
              <div className="flex items-center justify-between border-b border-pos-border-tertiary px-5 py-3">
                ...
                <span className="text-[11px] text-pos-text-tertiary">
                  by total payout
                </span>
              </div>
              <div className="px-5 py-4">
                ...
                {chartsQuery.isLoading ? (
                  <div className="space-y-4">
                    ...
                    {Array.from({ length: 4 }).map((_, i) => (
                      <div
                        key={i}
                        className="flex justify-between border-b border-pos-border-tertiary py-2"
                      >
                  ...
                  (() => {
                    ...
                    const accuracyPct =
                      total > 0
                        ? Math.max(
                            0,
                            Math.round((1 - r.avgErrorRate) * 100 * 10) / 10,
                          )
                        : 0;
                    const underpaidPct =
                      total > 0
                        ? Math.round((r.underpaidCount / total) * 100)
                        : 0;
                    const overpaidPct =
                      total > 0
                        ? Math.round((r.overpaidCount / total) * 100)
                        : 0;
                    const exactPct =
                      total > 0 ? Math.round((r.exactCount / total) * 100) : 0;
                    return (
                      <div className="space-y-4">
                        <div>
                          <div className="flex justify-between text-[13px]">
                            ...
                            <span className="font-medium text-pos-success">
                              {accuracyPct}%
                            </span>
                          </div>
                        ...
                        {[
                          {
                            label: "Avg. delta per order",
                            value: fmtGHS(r.avgDelta),
                          },
                          {
                          ...
                          >
                            ...
                            <span
                              className={cn(
                                "font-medium text-pos-text-primary",
                                row.valueClass,
                              )}
                            >
                              {row.value}
                        ...
                        {underpaidPct > 60 && (
                          <div className="rounded-[var(--radius-md)] bg-pos-warning-soft px-3 py-2 text-[12px] text-pos-warning">
                            Tip: consistently underpaying suggests estimated
                            rate is set too low. Consider revising the default
                            rate.
                          </div>
                        )}
                      </div>
