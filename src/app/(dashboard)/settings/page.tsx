...
export default function SettingsPage() {
  ...
  // Manage roles dialog
  const [manageTarget, setManageTarget] = useState<ManageRolesTarget | null>(
    null,
  );
  ...
  const rateQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch("/api/settings/rate").then((r) => r.json() as Promise<RateData>),
  });
  ...
  const isForbidden =
    usersQuery.data?.error === "Forbidden" || usersQuery.isError;
  ...
  const handleSaveRate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setRateError(data.error ?? "Failed to save rate");
        return;
      }
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await apiFetch(`/api/settings/users/${pendingDelete.userId}`, {
        method: "DELETE",
      });
      void queryClient.invalidateQueries({ queryKey: ["settings-users"] });
    } finally {
  ...
  const confirmRoleChange = async () => {
    ...
    try {
      if (pendingRoleChange.type === "grant") {
        ...
        await apiFetch(
          `/api/settings/users/${pendingRoleChange.userId}/roles`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      if (manageTarget) {
        ...
        const updated = freshUsers.data?.find(
          (u) => u.id === manageTarget.user.id,
        );
              ...
              <div className="overflow-hidden rounded-lg border border-pos-border-tertiary bg-pos-bg-primary">
                <div className="border-b border-pos-border-tertiary px-5 py-4">
                  ...
                  <p className="mt-0.5 text-[13px] text-pos-text-secondary">
                    The buying rate per gram of gold (GHS). Pre-fills on every
                    new order — staff can still override it per order.
                  </p>
                </div>
                <form onSubmit={handleSaveRate} className="px-5 py-5">
                  <Label
                    htmlFor="gold-rate"
                    className="text-[12px] font-medium text-pos-text-primary"
                  >
                    Rate (GHS / g)
                  </Label>
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    {rateSaved && (
                      <span className="text-[12px] text-pos-success">
                        Rate saved
                      </span>
                    )}
                    {rateError && (
                  ...
                  {rateQuery.data?.updatedAt && (
                    <p className="mt-3 text-[11px] text-pos-text-secondary">
                      Last updated{" "}
                      {new Date(rateQuery.data.updatedAt).toLocaleString(
                        "en-GB",
                        {
                          day: "2-digit",
                          month: "short",
                      ...
                      {rateQuery.data.updatedBy &&
                        ` · by ${rateQuery.data.updatedBy}`}
                    </p>
                  )}
                </form>
              </div>
              ...
              <div className="space-y-2">
                {usersQuery.isPending ? (
                  ...
                  users.map((u) => {
                    ...
                    const displayRoles = hasGrantedRoles
                      ? u.grantedRoles
                      : null;
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1 basis-40">
                          ...
                          <p className="truncate text-[12px] text-pos-text-secondary">
                            {u.email}
                            {u.phone && (
                              <span className="ml-2 text-pos-text-secondary opacity-60">
                                ·
                              </span>
                            )}
                            {u.phone && (
                ...
                <div className="border-t border-pos-border-tertiary pt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Select
                      ...
                      onValueChange={(v) =>
                        setGrantRoleValue(v as GrantableRole)
                      }
                    >
